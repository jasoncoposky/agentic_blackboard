...
std::string format_target_uri(const std::string& target) {
    ...
    if (target.starts_with("urn:")) return "<" + target + ">";
    if (target.starts_with("identity:")) return "<urn:asos:identity:" + sanitize_str(target.substr(9)) + ">";
    if (target.starts_with("project:")) return "<urn:asos:project:" + sanitize_str(target.substr(8)) + ">";
    return "<urn:asos:atom:" + sanitize_str(target) + ">";
}
...
std::string predicate_for_relation(const std::string& rel) {
    ...
    if (rel.starts_with("asos:") || rel.starts_with("rdfs:") || rel.starts_with("schema:") || rel.starts_with("prov:") || rel.starts_with("geo:")) {
        return rel;
    }
    return "asos:" + rel;
}
...
std::string RdfExporter::export_turtle(Blackboard* blackboard, uint32_t principal_id) {
    ...
    auto keys1 = store->get_prefix_keys_all_shards("n:", "n:", 10000);
    auto keys2 = store->get_prefix_keys_all_shards("n:{", "n:{", 10000);
    ...
    for (const auto& key : unique_keys) {
        if (key.ends_with(":meta")) continue;
            ...
            if (has_type && type == "IDENTITY") {
                ...
                std::string subject_uri = "<urn:asos:identity:" + sanitize_str(id_str) + ">";
                ...
                std::string subject_uri = "<urn:asos:project:" + sanitize_str(proj_str) + ">";
                ...
                try {
                    ...
                    std::string subject_uri = "<urn:asos:atom:" + sanitize_str(atom_id) + ">";
                    ...
                    if (!entry.header.origin.agent_id.empty()) {
                        std::string agent = entry.header.origin.agent_id;
                        if (agent.starts_with("identity:")) agent = agent.substr(9);
                        props.push_back({"prov:wasGeneratedBy", "<urn:asos:identity:" + sanitize_str(agent) + ">"});
                    }
                    if (!entry.header.origin.project_id.empty()) {
                        std::string proj = entry.header.origin.project_id;
                        if (proj.starts_with("project:")) proj = proj.substr(8);
                        props.push_back({"asos:belongsTo", "<urn:asos:project:" + sanitize_str(proj) + ">"});
                    }
